...
class StateHandler extends eventHandler {
  ...
  async onMessage(event) {
    ...
    if (message.includes("From ")) {
      // Hypixel format is usually: "From [Rank] Username: message" or "From Username: message"
      const msgWithoutPrefix = message.split("From ")[1];
      const username = msgWithoutPrefix.substr(0, msgWithoutPrefix.indexOf(":")).split(" ").pop().trim();
    
      this.command.handle(username, colouredMessage, false);
      return;
    }
    ...
    if (config.discord.channels.debugMode === true) {
      ...
      // Add a backslash before any @ symbol to prevent pinging (wsplash </3)
      let debugMessage = message.replace(/@/g, "\\@ ")
    ...
    if (this.isRequestMessage(message)) {
      ...
      bot.chat(`/oc ${username} has requested to join the guild!`)
      await delay(1000);
    ...
    if (this.isJoinMessage(message)) {
      ...
      await delay(3000);
      bot.chat(`/gc ${replaceVariables(messages.guildJoinMessage.replace("{username}", username), { prefix: config.minecraft.bot.prefix })} | by NoahBruhh and Teun, @duckysolucky, @xbqst`);
